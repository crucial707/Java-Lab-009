...
```text
The object is mutable because the object's state can be changed.
```
...
```text
The return value from #3 is a Point object, while the return value from #4 is a double.
```
...
```text
They are not aliased. p1 and p2 are not aliased because they are different objects.
```
